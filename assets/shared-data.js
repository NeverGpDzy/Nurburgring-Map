(function(global){
  ...
  function getNavigatorLanguages(){
    ...
    if(global.navigator && global.navigator.languages && global.navigator.languages.length){
      nav = nav.concat(global.navigator.languages)
    }
    if(global.navigator && global.navigator.language){
      nav.push(global.navigator.language)
    }
    return nav
  }
  ...
  function resolveLang(){
    ...
    if(forced === 'cn' || forced === 'en') return forced

    var stored = storageGet(LANG_KEY)
    if(stored === 'cn' || stored === 'en') return stored

    return getNavigatorLanguages().some(function(item){
      return /^zh-/i.test(item)
    }) ? 'cn' : 'en'
  }

  function setLang(lang, options){
    if(lang !== 'cn' && lang !== 'en') return
    global.lang = lang
    if(global.document && global.document.documentElement){
      global.document.documentElement.lang = lang === 'cn' ? 'zh-Hans' : 'en'
    }
  ...
  function resolveDarkMode(){
    var stored = storageGet(DARK_KEY)
  ...
  function setDarkMode(enabled){
    storageSet(DARK_KEY, enabled ? 'dark' : 'light')
  }
